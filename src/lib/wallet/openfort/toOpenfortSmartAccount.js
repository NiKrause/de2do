...
export async function toOpenfortSmartAccount({
  ...
  const signWithWebAuthn = async ({ userOperation }) => {
    ...
    const signature = encodeWebAuthnSignature(webauthnData, {
  ...
  const baseSmartAccount = toSmartAccount({
    ...
    async signUserOperation(parameters) {
      if (!owner?.signTypedData) {
        throw new Error('Owner account is required for signUserOperation; use signWithWebAuthn for delegated calls.');
      }
