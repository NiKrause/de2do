...
import { getAppChain, getEscrowAddress, getLocalDevFunderPrivateKey, getRpcUrl } from '../chain/config.js';
 ...
 */
async function getEscrowReleaseRowsForBeneficiary(client, escrowAddress, beneficiaryAddress, fromBlock, toBlock) {
  /** @type {AccountTxRow[]} */
  const rows = [];
...
export async function getRecentAccountTransactions(
  ...
  if (escrowAddr && escrowAddr.startsWith('0x')) {
    const fromBlock =
      latestBlockNumber > blockLookback ? latestBlockNumber - blockLookback : 0n;
    try {
