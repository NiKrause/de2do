...
import { concatHex, createClient, createPublicClient, createWalletClient, encodeAbiParameters, encodeFunctionData, http, keccak256, parseEther, toBytes, toHex } from 'viem';
...
import { getAppChain, getBundlerUrl, getEntryPointAddress, getImplementationAddress, getPaymasterUrl, getRpcUrl } from '../chain/config.js';
...
/** @type {const} */
const LOCAL_7702_PREPARE_PARAMETERS = [
  'blobVersionedHashes',
  'chainId',
  'fees',
  'nonce',
 ...
 */
async function sendAnvil7702PreparedRawTx({ signer, chain, rpcUrl, publicClient, authorizationList, callData }) {
...
function normalizeUserOperationForSend(userOperation, publicFeeEstimate) {
  ...
  const maxPriorityFeePerGas =
    ...
    publicFeeEstimate.maxPriorityFeePerGas ??
    1n;
...
function applyLocalDevGasBuffer(userOperation, chain) {
  ...
  return {
    ...userOperation,
    callGasLimit: userOperation.callGasLimit && userOperation.callGasLimit > 1_500_000n
...
async function buildSignedAuthorization({ chain, rpcUrl, signerAddress, privateKey, implementationAddress }) {
...
export async function consolidatePasskeyCredentials() {
  const walletCredential = loadPasskeyWalletCredential() || importWalletCredentialFromIdentityPasskey();
  if (!walletCredential) {
...
export async function sendPasskeyBundlerCall({ smartAccountClient, publicClient, calls, callData }) {
  const entryPointAddress = getEntryPointAddress();
  if (!entryPointAddress) throw new Error('VITE_ENTRY_POINT_ADDRESS is not configured');
  ...
  if (smartAccountClient.chain?.id === 31337) {
    ...
    if (!ownerPrivateKey) {
      throw new Error('Local owner key not found for smart account; recreate the smart account on Anvil.');
    }
