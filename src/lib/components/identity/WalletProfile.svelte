<script>
  ...
  import { getIdentityProfile, setWalletAddressForCurrentIdentity, setIdentityProfile } from '$lib/identity/profile.js';
  import {
  ...
  import { getAppChain, getBundlerUrl, getEntryPointAddress, getEscrowAddress, getImplementationAddress, getLocalDevFunderPrivateKey, getPaymasterUrl, getRpcUrl, isPaymasterEnabled } from '$lib/chain/config.js';
  ...
  onMount(async () => {
    ...
    missingEscrowConfig = /** @type {string[]} */ ([
      !getEscrowAddress() && 'VITE_ESCROW_CONTRACT'
    ].filter(Boolean));
    missingOptionalConfig = isPaymasterEnabled()
      ? /** @type {string[]} */ ([
          !getPaymasterUrl() && 'VITE_PAYMASTER_URL'
        ].filter(Boolean))
      : [];
  ...
  async function handleCreatePasskey() {
    try {
      ...
      if (!walletAddress || !walletAddress.startsWith('0x')) {
        showToast('Enter a valid 0x wallet address before creating a wallet passkey', 'warning', 2500);
        return;
      }
  ...
  async function handleCreateSmartAccount() {
    ...
    try {
      ...
      const localDirect7702Bootstrap = getAppChain().id === 31337 && !userOperation && Boolean(receipt?.transactionHash);
...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-sm" data-testid="wallet-profile">
  <div class="mb-2 text-sm font-semibold text-gray-700">Passkey Wallet Profile</div>
  <div class="text-xs text-gray-500">
    Link your DID to a wallet address and wallet passkey used for escrow payouts.
    Creating a smart account will generate a new address. If available, we reuse your existing identity passkey.
  </div>
  {#if identityPasskeyMode}
    <div class="mt-2 rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900">
      Identity passkey detected ({identityPasskeyMode} mode) for DID login.
      Wallet passkey is separate and used for smart-account signing.
    </div>
  {/if}
  ...
  {#if !hasCredential && identityPasskeyMode === 'worker'}
    <div class="mt-1 text-xs text-amber-700">
      Worker identity passkeys secure your DID keystore (ed25519) and usually cannot be reused as wallet
      signing keys. Create Wallet Passkey once to enable smart-account actions.
    </div>
  {/if}
  {#if missingConfig.length > 0}
  ...
  {#if implementationReady === false}
    <div class="mt-2 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800">
      Implementation contract is not deployed at `VITE_IMPLEMENTATION_CONTRACT` on the current chain.
    </div>
  {/if}
  ...
  <div class="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
    ...
    >
      {hasCredential ? 'Update Wallet Passkey' : identityPasskeyMode === 'worker' ? 'Create Wallet Passkey (Required)' : 'Create Wallet Passkey'}
    </button>
    <button
      type="button"
  ...
  <div class="mt-2 flex items-center gap-2 text-xs text-amber-800">
    ...
    <label for="smart-account-warning">
      I understand this will generate a new smart account address and override the wallet address field.
    </label>
  </div>
  {#if smartAccountAddress}
    <div
    ...
    <div class="mt-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800">
      <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-1 max-w-prose text-[11px] leading-snug text-slate-600">
            This is not your “net worth” across escrow. After <strong>Lock funds</strong>, ETH leaves this address
            and sits in the <strong>TodoEscrow</strong> contract until <strong>Confirm &amp; Pay</strong> (release) or
            refund. As beneficiary, your balance here rises on release, not when Alice locks.
          </p>
          ...
          <div class="mt-2 space-y-2">
            {#each recentTransactions as tx (tx.hash)}
              <div
                ...
                title={tx.hash}
              >
                <div class="font-mono text-[11px] text-slate-900">Tx {shortenHash(tx.hash, 14, 10)}</div>
                <div class="mt-1 space-y-0.5 font-mono text-[10px] leading-snug break-all text-slate-800">
  ...
  {#if smartAccountDebug}
    <details class="mt-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-800">
      <summary class="cursor-pointer font-semibold">Debug: EIP-7702 init payload</summary>
      <pre class="mt-2 overflow-auto rounded bg-slate-900 p-2 text-[11px] text-slate-100">{stringifyDebug(smartAccountDebug)}</pre>
    </details>
  {/if}
